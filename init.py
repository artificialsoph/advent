...
MAKE_CODE_TEMPLATE = False  # Set to false to not make code templates. Note that even if OVERWRITE is set to True, it will never overwrite codes.
...
last_advent_of_code_year = (
    2022  # The setup will download all advent of code data up until that date included
)
...
config.read("SECRET")
USER_SESSION_ID = config["AOC"]["USER_SESSION_ID"]
